...
const sections: Section[] = [
  {
    title: "Strategy",
    description:
      "AI-ready organizations have a proactive, customer-focused AI strategy that is clearly communicated. They are clear on their AI vision, able to influence key stakeholders, and have a defined process for identifying and prioritizing high-value AI projects.",
    instructions:
      'Please rate each statement on a scale of 1 to 5 where 1 indicates "Strongly disagree" and 5 indicates "Strongly agree".',
    questions: [
  ...
  },
  {
    title: "Execution",
    description:
      "AI-ready organizations lead with clarity and agility. They bring together teams from across departments, invest in employee upskilling, and have repeatable models to scale successful AI initiatives.",
    instructions:
      'Please rate each statement on a scale of 1 to 5 where 1 indicates "Strongly disagree" and 5 indicates "Strongly agree".',
    questions: [
  ...
  },
  {
    title: "Innovation",
    description:
      "AI-ready organizations foster a culture of experimentation and innovation. They explore disruptive use cases and fund promising projects.",
    instructions:
      'Please rate each statement on a scale of 1 to 5 where 1 indicates "Strongly disagree" and 5 indicates "Strongly agree".',
    questions: [
  ...
  },
  {
    ...
    description:
      "AI-ready organizations build strong foundations to support AI adoption. They ensure their data systems are robust and accessible, their teams are developing in-house AI skills, and they follow ethical frameworks that guide responsible use of AI.",
    instructions:
      'Please rate each statement on a scale of 1 to 5 where 1 indicates "Strongly disagree" and 5 indicates "Strongly agree".',
    questions: [
  ...
  },
  {
    title: "Impact & Vision",
    description:
      "AI-ready organizations not only adopt AI — they define what success looks like.",
    instructions:
      'Please rate each statement on a scale of 1 to 5 where 1 indicates "Strongly disagree" and 5 indicates "Strongly agree".',
    questions: [
...
const SCRIPT_URL =
  "https://script.google.com/macros/s/AKfycbz7BmwwKgVdzl7LKa1_KlW_GLcw0gnzLYYXN-yzo0X0-QLRtbF0fkBGg9q9YX-5sRli/exec";
  ...
  <div className="text-center p-6">
    ...
    >
      ...
      <p className="text-xs text-gray-400 mt-2">
        You will be redirected automatically in 5 seconds after download.
      </p>
    </motion.div>
  </div>
);
...
export default function AIReadinessAssessment() {
  ...
  const questionIndexOffset = sections
    .slice(0, step)
  ...
  const isStepValid = (): boolean => {
    return currentSection.questions.every(
      (_, i) => responses[`q${questionIndexOffset + i + 1}`],
    );
  };
  ...
  const downloadPDF = async () => {
    ...
    const logo = await fetch(
      "https://crispai.crispvision.org/media/crisp-logo.png",
    )
      .then((res) => res.blob())
      .then(
        (blob) =>
    ...
    sections.forEach((section: any, secIndex: number) => {
      ...
      section.questions?.forEach((question: string, qIndex: number) => {
        ...
        if (y > 270) {
          // ✅ Check after every question/answer
          doc.addPage();
          y = 20;
        }
    ...
    doc.text("Generated by CrispAI — Empowering AI Readiness", 105, 290, {
      align: "center",
    });
    ...
    <div className="min-h-screen bg-[#f4faff] px-4 py-10 text-[#003366] font-sans relative overflow-hidden">
      ...
      {["Top", "Bottom", "Left", "Right"].map((position) => (
        <div
          ...
          className={`absolute ${
            position === "Top"
              ? "top-0 left-0 w-full rotate-180"
              : position === "Bottom"
                ? "bottom-0 left-0 w-full"
                : position === "Left"
                  ? "left-0 top-0 h-full w-40 rotate-90 -translate-x-1/2"
      ...
      >
        <div className="flex flex-col items-start mb-6 gap-2">
          ...
          />
          <h1 className="text-2xl font-extrabold text-[#003366] leading-tight">
            AI Readiness
          </h1>
                ...
                {step < sections.length ? (
                  ...
                  <div className="space-y-6">
                    <h2 className="text-lg font-bold mb-4">
                      Select Your Primary AI Use Cases
                    </h2>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {useCases.map((usecase) => (
                        <UseCaseOption
                          key={usecase}
                          usecase={usecase}
                          selected={(responses.usecases || []).includes(
                            usecase,
                          )}
                          onChange={(checked) => {
                            setResponses((prev) => {
